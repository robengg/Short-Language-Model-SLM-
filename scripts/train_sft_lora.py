...
def main():
    ...
    def fmt(ex): return TEMPLATE.format(**ex)
    ds_tok = ds["train"].map(lambda b: tok(fmt(b), truncation=True, max_length=args.block_size),
    ...
    Trainer(model=model, args=targs, train_dataset=ds_tok, data_collator=collator).train()
